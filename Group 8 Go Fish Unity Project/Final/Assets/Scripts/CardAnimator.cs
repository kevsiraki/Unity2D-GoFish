...
namespace GoFish {
    ...
    public class CardAnimator : MonoBehaviour { //card animations
        ...
        public UnityEvent OnAllAnimationsFinished = new UnityEvent(); // invoked when all queued card animations have been played
        bool working = false;
        ...
        public void DealDisplayingCards(Player player, int numberOfCard, bool animated = true) {
            ...
            for (int i = start; i > finish; i--) {
                ...
                if (animated) AddCardAnimation(card, player.NextCardPosition());
                else card.transform.position = player.NextCardPosition();
                
            }
            foreach (Card card in cardsToRemoveFromDeck) DisplayingCards.Remove(card);
        }
        ...
        private void Update() {
            ...
            else if (currentCardAnimation.Play())
			{  
				NextAnimation();
				audioData.Play();
			}
