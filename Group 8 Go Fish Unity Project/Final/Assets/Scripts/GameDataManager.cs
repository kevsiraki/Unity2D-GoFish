...
namespace GoFish {
    ...
    public class GameDataManager {
        ...
        public GameDataManager(Player local, Player remote, string roomId = "1234567890123456") { //dummy ID
            localPlayer = local;
            remotePlayer = remote;
        ...
        public void Shuffle() {
            ...
            for (byte value = 0; value < 52; value++) cardValues.Add(value);
        ...
        public Player Winner() {
            ...
            if (winnerPlayerId.Equals(localPlayer.PlayerId)) return localPlayer;
        ...
        public List<byte> TakeCardValuesWithRankFromPlayer(Player player, Ranks ranks) {
            ...
            foreach (byte cv in playerCards) if (Card.GetRank(cv) == ranks) result.Add(cv);
        ...
        public Dictionary<Ranks, List<byte>> GetBooks(Player player) {
            ...
            if (groups.Count() > 0) {
                ...
                foreach (var group in groups) {
                    List<byte> cardValues = new List<byte>();
                    foreach (var value in group) cardValues.Add(value);
        ...
        public Player GetCurrentTurnPlayer() {
            ...
            if (localPlayer.PlayerId.Equals(playerId)) return localPlayer;
            else return remotePlayer;
        }

        public Player GetCurrentTurnTargetPlayer() {
            string playerId = protectedData.GetCurrentTurnPlayerId();
            if (localPlayer.PlayerId.Equals(playerId)) return remotePlayer;
