...
namespace GoFish {
    public class Game : MonoBehaviour {
		...
		public int NrBackground;
		public void ChoosenColor ()
		{
			PlayerPrefs.SetInt("ColorOfTheBackground", NrBackground);
		}
        ...
        //****************** Game Flow *********************//
        public virtual void GameFlow() {//update gamestate
            if (gameState > GameState.GameStarted) {
                CheckPlayersBooks();
                ShowAndHidePlayersDisplayingCards();
                if (gameDataManager.GameFinished()) gameState = GameState.GameFinished;
            }
            switch (gameState) {
                case GameState.Idle:
                    {
        ...
        public void OnTurnSelectingNumber() {
            ...
            if (currentTurnPlayer == localPlayer) SetMessage($"Player's turn. Select a card from your deck.");
            else SetMessage($"{currentTurnPlayer.PlayerName}'s turn");
        ...
        protected virtual void OnTurnConfirmedSelectedNumber() {
            if (currentTurnPlayer == localPlayer) SetMessage($"Asking {currentTurnTargetPlayer.PlayerName} for {selectedRank}s...");
            else SetMessage($"{currentTurnPlayer.PlayerName} is asking for {selectedRank}s...");
        ...
        protected virtual void OnTurnGoFish() {
            ...
            if (Card.GetRank(cardValue) == selectedRank) cardAnimator.DrawDisplayingCard(currentTurnPlayer, cardValue);
            else {
        ...
        public void OnGameFinished() {
            if (gameDataManager.Winner() == localPlayer) {
				SetMessage($"Player Wins! Press 'M' for menu.");
				audioData.Play();
			}
            else { 
				SetMessage($"AI Wins! Press 'M' for menu."); 
				audioData.Play();
			}
        }
        //****************** Helper Methods *********************//
